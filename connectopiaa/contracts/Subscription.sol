...
contract Subscription is Post {
    ...
    modifier hasEnoughFound(uint256 _postId) {
        require(msg.value >= posts[_postId].price, "You don't have sufficiant found to subscribe.");
        _;
    }

    function payForPost(uint256 _postId) public payable doesPostExist(_postId) isContentPayable(_postId) hasEnoughFound(_postId) returns(bool) {
